...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'Refactor.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'Refactor.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'Refactor.Client.ProtectedToken'
