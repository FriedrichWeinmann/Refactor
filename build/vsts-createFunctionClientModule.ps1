...
$parentModule = 'Refactor'
if (-not $ModuleName) { $ModuleName = 'Refactor.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\Refactor'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\Refactor.psd1"
$prereqHash = @{
